...
# at https://withoutbg.diegochagas.com — multipart upload to
...
from gi.repository import Gimp, GimpUi, Gtk, GObject, GLib, Gio, Gegl
...
SERVER_URL = "https://withoutbg.diegochagas.com"
...
USER_AGENT = 'WithoutBG-GIMP/1.0'


def _health_check(server_url):
    """Return (ok: bool, info: dict | str)."""
    try:
        # An explicit User-Agent: reverse proxies commonly reject the
        # default Python-urllib one (403).
...
def _post_image(png_bytes, server_url, output_type):
    ...
    gimp-setup patch: the self-hosted WithoutBG API expects a multipart
    "file" upload at /api/remove-background and always returns the RGBA
    cutout (*output_type* is not supported — the caller converts the
    cutout to a matte locally).
    """
    boundary = '----gimp-setup-' + os.urandom(12).hex()
    body = (
        b'--' + boundary.encode() + b'\r\n'
    ...
    url = f"{server_url.rstrip('/')}/api/remove-background"
    ...
    try:
        ...
        try:
            ...
            msg = detail.get('error') or detail.get('detail') or str(exc)
        except Exception:
            msg = str(exc)
...
def _cutout_to_matte(matte_img):
    """Convert an RGBA cutout into a grayscale matte, in place.

    gimp-setup patch: the self-hosted API returns the cutout (RGBA with
    the background removed) instead of the single-channel matte. The
    matte is rebuilt locally from the cutout's alpha channel: alpha →
    selection → white-on-black fill, preserving partial transparency.
    """
    layer = matte_img.get_layers()[0]
    if not layer.has_alpha():
        return  # already a matte

    width, height = matte_img.get_width(), matte_img.get_height()
    Gimp.context_push()
    try:
        matte_img.select_item(Gimp.ChannelOps.REPLACE, layer)
        sel_chan = Gimp.Selection.save(matte_img)

        matte_layer = Gimp.Layer.new(matte_img, 'matte', width, height,
                                     Gimp.ImageType.RGB_IMAGE, 100,
                                     Gimp.LayerMode.NORMAL)
        matte_img.insert_layer(matte_layer, None, -1)

        Gimp.Selection.none(matte_img)
        Gimp.context_set_foreground(Gegl.Color.new('black'))
        matte_layer.edit_fill(Gimp.FillType.FOREGROUND)

        matte_img.select_item(Gimp.ChannelOps.REPLACE, sel_chan)
        Gimp.context_set_foreground(Gegl.Color.new('white'))
        matte_layer.edit_fill(Gimp.FillType.FOREGROUND)

        Gimp.Selection.none(matte_img)
        matte_img.remove_channel(sel_chan)
        matte_img.flatten()
    finally:
        Gimp.context_pop()
...
def _server_status_markup(server_url):
    ...
    if ok:
        if isinstance(info, dict):
            model = info.get('model') or info.get('service', '?')
...
def run_remove_background(procedure, run_mode, image, drawables, config, data):
    ...
    try:
        ...
        try:
            ...
            # 2 — send to server.
            # The self-hosted API returns the RGBA cutout at the (≤1024 px)
            # input size; the matte is rebuilt locally from its alpha and
            # upscaled to the full-resolution layer — no quality loss.
            ...
            # gimp-setup patch: the self-hosted API returns an RGBA
            # cutout; rebuild the grayscale matte from its alpha channel.
            _cutout_to_matte(matte_img)
...
class WithoutBGPlugin(Gimp.PlugIn):
    ...
    def do_create_procedure(self, name):
        ...
        proc.set_documentation(
            'Remove background via a WithoutBG server',
            'Sends the active layer to a WithoutBG server, rebuilds the alpha '
            'matte from the returned cutout, and adds it as an unapplied '
            'layer mask.',
            name,
        )
